...
class AckermanPerturbation(Perturbation):
    ...
    def perturb(self, history_frames: np.ndarray, future_frames: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if num_history_frames > 1:
            curr_speed = np.linalg.norm(np.diff(trajectory[curr_frame_idx - 1:curr_frame_idx + 1, :2], axis=0))
        else:
            curr_speed = displacements[0]

        if np.random.rand() >= self.perturb_prob:
            return history_frames.copy(), future_frames.copy(), curr_speed
        ...
        v0 = curr_speed * speed_multiplier + speed_offset
